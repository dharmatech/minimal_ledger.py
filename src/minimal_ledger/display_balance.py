...
# def ledger_all_balances(ledger: Ledger) -> Dict[str, Decimal]:
#     accounts = ledger_all_accounts(ledger)
#     return {
#         account: sum(entry.amount for entry in ledger.entries() if entry.description.startswith(account))
#         for account in accounts
#     }

def ledger_all_account_with_values(ledger: Ledger) -> Dict[str, Decimal]:
    accounts = ledger_all_accounts(ledger)
    return {
        account: sum(entry.amount for entry in ledger.entries() if entry.description.startswith(account))
        for account in accounts
...
def display_balance(ledger: Ledger, output_type='terminal'):
    accounts = ledger_all_accounts(ledger)
    ...
    for account in accounts:
        ...
        total = sum(entry.amount for entry in ledger.entries() if entry.description.startswith(account))
        ...
        if output_type == 'terminal':
            output += f"{account:<{max_account_length}}: {total:>7}\n"
